...
```bash
# 全量回归（33 项，必过；约 60-90s，大头是 [20]/[30] 两个 AI 对战回归）
...
5. **数值调整只改 settings.py**，改完跑 [20]（总胜率 45-55%）和 [30]（分对阵 35-65%，
   报警是警示不是失败——当前已知 garnet vs verdant ~67% 在报警中）。
...
[31] 录像回放一致 · [32] 第四机甲 · [33] 街机流程。

## 当前状态与遗留（2026-08-30）

- 路线图阶段 0-8 全部完成（见 DESIGN_ROADMAP.md 第五节勾选与第六节规划）。
- 遗留：① garnet vs verdant 对阵 ~67% 在 [30] 报警中（带缘震荡，调参第一入口）；
