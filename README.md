...
手柄无需配置：1 号手柄 → P1，2 号 → P2；十字键/左摇杆移动，按钮 0/1/2/3 = 斩/束/投/超。
...
- **超必杀槽**：命中 +12 / 受击 +8 / 格挡 +4，满 100 发动，自带定格演出 + 全屏白闪 + 发动无敌。
...
```
main.py        入口：场景机（菜单/选人/对战/键位/结算）、回合流程、判定结算、
               KO 回放、训练模式、手柄输入合并、自测（--selftest）
